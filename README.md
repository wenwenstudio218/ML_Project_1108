...
### 🏆 關鍵成果
* **預測成效：** 表現最好的模型 **Recall (召回率) 達 0.83**，能精準找出超過八成的潛在離職者。
...
* **[👉 點擊查看完整 Live Demo https://ml-project-turnover-intention.onrender.com/]**
...
三層式架構-強韌分析圖：
![專案架構強韌分析圖](./images/Robustness-Diagram.png)
...
### 1. 資料前處理與挑戰
* **資料來源：** 勞動部「勞工生活及就業狀況調查(111~113年)」。
* **挑戰 (Challenge)：** 原始數據存在嚴重**類別不平衡 (Imbalanced Data)**，離職傾向樣本僅佔 15.3%。
* **解決方案：**
    * 設定 `class_weight='balanced'` 自動調整權重，讓模型加權關注少數類別。
...
[👉 點擊查看「邏輯斯迴歸」模型Colab訓練過程](./models/logistic_model_train.ipynb)  

[👉 點擊查看「隨機森林」模型Colab訓練過程](./models/rf_model_train.ipynb)  
